...
anything predictive (see the peer-review bar in [`../../THESIS.md`](../../THESIS.md) §6, and the
...
1. **As-of cutoff (no look-ahead).** Every feature must exist at `feature_as_of`, which must sit on
   ...
   any row that violates this — so "structure choice is not pinned" (README issue #10) becomes a
   *leakage* check here, not a reproducibility nit.
...
- **Corpus** — reconstruct `(trial design, features, outcome, readout_date)` point-in-time over
  ...
  withdrawn trials kept in. This is the laborious, high-value part (THESIS §6, assumption 2).
...
- **Sponsor→ticker** — real entity resolution (README issue #7) to move from a watchlist to a
  universe; breadth is the whole thesis.
